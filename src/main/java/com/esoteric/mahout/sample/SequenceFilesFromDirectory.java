...
public class SequenceFilesFromDirectory extends AbstractJob {
	...
	private static final String PREFIX_ADDITION_FILTER = AvroPrefixAdditionFilter.class
			.getName();
	...
	public int run(String[] args) throws Exception {
		...
		ChunkedWriter writer = new ChunkedWriter(conf, Integer.parseInt(options
				.get(CHUNK_SIZE_OPTION[0])), output);

		try {
			SequenceFilesFromDirectoryFilter pathFilter;
			String fileFilterClassName = options
					.get(FILE_FILTER_CLASS_OPTION[0]);
			if (AvroPrefixAdditionFilter.class.getName().equals(
					fileFilterClassName)) {
				pathFilter = new AvroPrefixAdditionFilter(conf, keyPrefix,
						options, writer, charset, fs);
			} else {
				Class<? extends SequenceFilesFromDirectoryFilter> pathFilterClass = Class
						.forName(fileFilterClassName).asSubclass(
								SequenceFilesFromDirectoryFilter.class);
				Constructor<? extends SequenceFilesFromDirectoryFilter> constructor = pathFilterClass
						.getConstructor(Configuration.class, String.class,
								Map.class, ChunkedWriter.class, Charset.class,
								FileSystem.class);
	...
	protected void addOptions() {
		...
		addOption(CHUNK_SIZE_OPTION[0], CHUNK_SIZE_OPTION[1],
				"The chunkSize in MegaBytes. Defaults to 64", "64");
		...
		addOption(KEY_PREFIX_OPTION[0], KEY_PREFIX_OPTION[1],
				"The prefix to be prepended to the key", "");
		addOption(
				CHARSET_OPTION[0],
				CHARSET_OPTION[1],
				"The name of the character encoding of the input files. Default to UTF-8",
				"UTF-8");
	}

	/**
	 * Override this method in order to parse your additional options from the
	 * command line. Do not forget to call super() otherwise standard options
